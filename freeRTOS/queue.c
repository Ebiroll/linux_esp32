...
BaseType_t xQueueGenericReset( xQueueHandle pxQueue, int xNewQueue )
{
    //xQueueHandle * const pxQueue = ( xQueueHandle * ) xQueue;
	...
	taskENTER_CRITICAL();
	{
		...
		pxQueue->uxMessagesWaiting = ( unsigned int ) 0U;
		pxQueue->pcWriteTo = pxQueue->pcHead;
		pxQueue->pcReadFrom = pxQueue->pcHead + ( ( pxQueue->uxLength - ( unsigned int ) 1U ) * pxQueue->uxItemSize );
